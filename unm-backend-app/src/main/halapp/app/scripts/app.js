...
var toInject = [ 'ngCookies', 'ngResource', 'ngRoute', 'ngSanitize', 'hateoas',
		'ui.bootstrap', 'angular-loading-bar', 'angularSpinner', 'validator',
		'validator.rules', 'textAngular' ];
...
halApp
		.config([
				...
				'$validatorProvider',
				function($routeProvider, HateoasInterceptorProvider,
						HateoasInterfaceProvider, $validatorProvider) {
					$routeProvider.when('/main', {
						templateUrl : 'views/main.html',
						controller : 'MainCtrl',
						resolve : {
							managedEntity : function() {
								return {
									showsAll : true,
						...
						controller : 'CtrlNotifications'
					}).when('/notifications/:itemId/edit', {
						templateUrl : 'views/notification-edit.html',
						controller : 'CtrlNotificationEdit'
					}).when('/feeds', {
						templateUrl : 'views/feeds.html',
						controller : 'CtrlFeeds'
					}).when('/feeds/:itemId/edit', {
						templateUrl : 'views/feed-edit.html',
						controller : 'CtrlFeedEdit'
					}).when('/links', {
						templateUrl : 'views/links.html',
						controller : 'CtrlLinks'
					}).when('/links/:itemId/edit', {
						templateUrl : 'views/link-edit.html',
						controller : 'CtrlLinkEdit'
					}).when('/menus', {
						templateUrl : 'views/menus.html',
						controller : 'CtrlMenus'
					}).when('/menus/:itemId/edit', {
						templateUrl : 'views/menu-edit.html',
						controller : 'CtrlMenuEdit'
					}).when('/university-libraries', {
						templateUrl : 'views/university-libraries.html',
						controller : 'CtrlUniversityLibraries'
					}).when('/university-crous', {
						templateUrl : 'views/university-crous.html',
						controller : 'CtrlUniversityCrous'
					}).otherwise({
						redirectTo : '/main'
					});
						...
						taRegisterTool('uploadImage', {
							...
							action : function() {
								...
								openImageUploadModal(function(imageLink) {
									editor().wrapSelection('insertImage',
											imageLink, true);
								});
								return true;
							}/*
								 * , onElementSelect: { element: 'img', action:
								 * imgOnSelectAction }
								 */
						});
...
halApp.directive('optionsClass', function($parse) {
	return {
		require : 'select',
            ...
            scope.$watchCollection(optionsSourceStr, function (items) {
                scope.$$postDigest(function () {
                    ...
                    angular.forEach(items, function (item, index) {
						...
						if (!item.active) {
							angular.element(option).addClass(attrs.optionsClass);
						}
					});
                });
            });
