...
public interface CommentRepository extends JpaRepository<Comment, Long> {
	...
	@PreAuthorize(value="hasRole('superadmin') or (hasRole('admin') and principal.university.id == c.poi.university.id)")
	@Modifying
	@Query("update Comment c set c.active = :status where c.id = :commentId")
	int setStatus(@Param("commentId") Long commentId, @Param("status") Boolean status);
}
